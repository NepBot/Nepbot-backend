      ...
      for (const role of _conInfos.roles) {
        try {
          ...
          if (mapAstrodaoDiscord.action == 'remove' && memberInGuild.roles.cache.some(r => r.id === role.role_id)) {
            await memberInGuild.roles.remove(role.role_id).then(logger.info(`${memberInGuild.user.username} add role, the role name is ${role.name} in astrodao_task`)).catch(e => logger.error(e));
          }
